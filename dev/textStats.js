...
function measureTextSelection1() {

  const emojiList = `💸 🤑 🔥 😂 💥 🍻 🎉 🕺 💃 🎊 🍾 🥂 🥳 🎈 💶 💰 💎 👑 💍 🎁 🥇 💲 🌟 🚀 🙌 ⚡️ ❤️‍🔥 💫 🚨 🤯 ✨ 🤩 🏎 🔟 🦄 🌈 ❓ 🍀 💪 🌋 🏖 📛 😍 💩 👨‍💻 🧑‍💻 👩‍💻 🌎 🕸 👁 👁‍🗨 → ← ★ ‼️ ❗️ 💋 🍆 🍑 🌶 💥 🍬 🍭 🎂 🍫 🏄‍♂️ 😵‍💫 🤡 ☄️ 🍦 🎨 💷 💴 💵 🧨 💣 💹 🔊 🇺🇸 🌜 🌛 🌝 🌞 🐄 🎰 🍒 🪙 🔔 🍉 🍇 🍋 🎄 🍄 ❄️ 💦 📈`
    .split(' ')
  const generateContent1 = () => {
  ...
  times(SAMPLE_SIZE, t => {
    ...
    const emojiCount = content.reduce((t, c) => emojiList.includes(c) ? t+1 : t, 0)
    const emojiPortion = emojiCount / content.length

    if (emojiPortion > 0) hasEmojis++
    emojiSelections += emojiCount
    totalSelections += content.length
    emojiPortions += emojiPortion
  })

  console.log('===== STRATEGY 1 =====')
  console.log('% of mints with at least 1 emoji: ', hasEmojis/SAMPLE_SIZE)
  console.log('% of total selections with emojis: ', emojiSelections/totalSelections)
  console.log('Avgerage emoji portion: ', emojiPortions/hasEmojis)
  console.log('=======================')
}
...
function measureTextSelection2() {
  ...
  const explosion1 = emojis(`💥 🤯 🌋 ☄️ 🧨 💣`)
  const explosionFull = [...explosion1, ...energy]
  ...
  const emojiList = emojiLists.flat().map(e => e)
...
//   const textPrb = textLists.map(c => [c.length, c.flat()])
//   const emojiPrb = emojiLists.map(c => [c.length, c.flat()])
//   const pairingPrb = pairings.map(c => [c.length, c.flat()])
// console.log(textPrb, emojiPrb, pairingPrb)
...
measureTextSelection1()
measureTextSelection2()
